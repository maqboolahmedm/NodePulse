...
# ============================================================
# WALLET CONFIG — Fill in your wallet details
# Remove or leave empty any wallets you don't use
# ============================================================
...
# --- Wallet 3 (optional — leave empty if not used) ---
WALLET_3_ADDRESS=""
WALLET_3_PASSWORD=""
WALLET_3_LICENSES=()

# --- Wallet 4 (optional — leave empty if not used) ---
WALLET_4_ADDRESS=""
WALLET_4_PASSWORD=""
WALLET_4_LICENSES=()
...
# ============================================================
# Per-License Config
# Add one entry per license across ALL wallets
# Format: NODE_WALLET[LICENSE]="wallet_address"
#         NODE_PORT[LICENSE]=port_number
#         NODE_STORAGE[LICENSE]="/path/to/storage"
#         NODE_RPC[LICENSE]="https://your-rpc-endpoint"
# ============================================================
...
# ============================================================
# Build combined license list from all active wallets
# ============================================================
ALL_LICENSES=()
...
set_penalty_count() {
  sed -i "/^${1}=/d" "$PENALTY_FILE" 2>/dev/null
  echo "${1}=${2}" >> "$PENALTY_FILE"
}

increment_penalty() {
  local NEW=$(( $(get_penalty_count "$1") + 1 ))
  set_penalty_count "$1" "$NEW"; echo "$NEW"
}

reset_penalty() {
  set_penalty_count "$1" "0"
  log "✅ Node $1 penalties reset to 0"
}

# ============================================================
# Restart Node — uses per-wallet password
# ============================================================
restart_node() {
  local LICENSE="$1"
  local WALLET="${NODE_WALLET[$LICENSE]}"
  ...
  log "Restarting node $LICENSE [${WALLET_LABEL}] (offline ~${OFFLINE_DURATION})..."
  ...
  if is_node_running "$LICENSE"; then
    ...
    log "✅ Node $LICENSE [${WALLET_LABEL}] restarted (PID: $NEW_PID)"
    send_telegram "✅ <b>Node Restarted</b>
🔑 License: <code>${LICENSE}</code> [<b>${WALLET_LABEL}</b>]
...
📍 Host: $(hostname)"
    ...
    log "❌ Node $LICENSE [${WALLET_LABEL}] FAILED to restart!"
...
send_heartbeat() {
  ...
  for LICENSE in "${ALL_LICENSES[@]}"; do
    ...
    if is_node_running "$LICENSE"; then
      ...
      local LINE="🟢 <code>${LICENSE}</code> — PID <code>${PID}</code> — Up: <b>${UP}</b> — R: <b>${RC}</b> — ${PEN_ICON} P: <b>${PEN}/${PENALTY_MAX}</b>\n"
      save_pid "$LICENSE" "$PID"
      update_last_seen "$LICENSE"
    else
      local LINE="🔴 <code>${LICENSE}</code> — <b>DOWN</b> — R: <b>${RC}</b>\n"
    fi
...
log "Active wallets: $([ -n "$WALLET_1_ADDRESS" ] && echo W1) $([ -n "$WALLET_2_ADDRESS" ] && echo W2) $([ -n "$WALLET_3_ADDRESS" ] && echo W3) $([ -n "$WALLET_4_ADDRESS" ] && echo W4)"
